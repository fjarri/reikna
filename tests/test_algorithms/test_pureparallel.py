...
class NestedPureParallel(Computation):
    def __init__(self, size, dtype):
        Computation.__init__(
            self,
            [
                Parameter("output", Annotation(ArrayMetadata(dtype=dtype, shape=size), "o")),
                Parameter("input", Annotation(ArrayMetadata(dtype=dtype, shape=size), "i")),
            ],
        )
        ...
        self._p = PureParallel(
            [
                Parameter("output", Annotation(ArrayMetadata(dtype=dtype, shape=size), "o")),
                Parameter("i1", Annotation(ArrayMetadata(dtype=dtype, shape=size), "i")),
                Parameter("i2", Annotation(ArrayMetadata(dtype=dtype, shape=size), "i")),
            ],
            """
...
def test_guiding_input(queue):
    ...
    p = PureParallel(
        [
            Parameter("output", Annotation(ArrayMetadata(dtype=dtype, shape=(2, N)), "o")),
            Parameter("input", Annotation(ArrayMetadata(dtype=dtype, shape=N), "i")),
        ],
        """
...
def test_guiding_output(queue):
    ...
    p = PureParallel(
        [
            Parameter("output", Annotation(ArrayMetadata(dtype=dtype, shape=N), "o")),
            Parameter("input", Annotation(ArrayMetadata(dtype=dtype, shape=(2, N)), "i")),
        ],
        """
...
def test_from_trf(queue, guiding_array):
    ...
    arr_t = ArrayMetadata(dtype=dtype, shape=N)
