...
import numpy
from grunnur import API, Array, ArrayMetadata, Context, Queue, Snippet, dtypes
...
# Test array
arr = numpy.random.default_rng().integers(0, 10**6, 20000)


# A transformation that creates initial minmax structures for the given array of integers
to_mmc = Transformation(
    [
        Parameter("output", Annotation(ArrayMetadata(arr.shape, mmc_dtype), "o")),
        Parameter("input", Annotation(ArrayMetadata.from_arraylike(arr), "i")),
    ],
    """
    ${output.ctype} res;
...
reduction = Reduce(to_mmc.parameter.output, predicate)
reduction.parameter.input.connect(to_mmc, to_mmc.parameter.output, new_input=to_mmc.parameter.input)
...
assert minmax["cur_min"] == arr.min()  # noqa: S101
assert minmax["cur_max"] == arr.max()  # noqa: S101
